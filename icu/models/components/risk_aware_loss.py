...
class RiskAwareAsymmetricLoss(nn.Module):
    ...
    def forward(self, x, y, risk_coef: torch.Tensor):
        """
        x: logits [B, C]
        y: targets [B, C]
        ...
        # 5. Base ASL Loss (Unreduced)
        asl_loss = -one_sided_w * (loss_pos + loss_neg) # [B, C]
        
        # 6. Apply Critical Penalty (Dynamic)
        # We want weight = 1.0 when risk=0, and weight = critical_multiplier when risk=1.
        # multiplier logic: 1.0 + risk * (critical_multiplier - 1.0)
